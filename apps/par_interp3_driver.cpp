...
void test4(int * N){
  ...
  Real h[3]; // grid size for each dimension
  h[0]=1./N[0];
  h[1]=1./N[1];
  h[2]=1./N[2];
  int ref=0;
  ...
  long Q=isize[0]*isize[1]*isize[2]; // The number of query points
  int data_dof=3; // Three velocity fields
  PCOUT<<"Input parameters:\n";
  PCOUT<<"Global Size="<<N[0]<<"x"<<N[1]<<"x"<<N[2]<<" distributed among "<<nprocs<<" processors\n";
  PCOUT<<"Number of Query Points per process= "<<Q<<"\n";
  ...
  // initialize function values at grid points
  // Note that f is a vector in data_dof dimensions
  // We need to initialize each vector component on the computational
  // grid. Each parallel process owns  isize[0]xisize[1]xisize[2] of the
  // global domain (which is N[0]xN[1]xN[2])
  for(long i0=0;i0<isize[0];i0++)
    for(long i1=0;i1<isize[1];i1++)
      for(long i2=0;i2<isize[2];i2++){
        Real c[COORD_DIM]={ (i0+istart[0])*1.0/N[0],
  ...
  // Get ghost cells of f, and store the resutls into ghost_f
  // This is needed because the interpolation kernel requires
  // access to ghost point cells
  int g_size=3;
  int isize_g[3],istart_g[3];
  size_t g_alloc_max=accfft_ghost_xyz_local_size_dft_r2c( plan,g_size,isize_g,istart_g);
  Real *ghost_f=(Real*)accfft_alloc(g_alloc_max*data_dof);
  ...
  /*  First we need to initialize a plan for the interpolation.
   *  This plan has three phases that has to be executed in order:
   *  1. allocate: Which makes the necessary internal allocations and
   *  has to be called only once.
   *  2. scatter: Which distributes non local query points to a processor
   *  that owns the corresponding local grid. This has to be called once as
   *  well.
   *  3. interpolate: This is the main part of the plan which performs the interpolation
   *  and where most of the computational time is spent on.
   * */

  //time+=-MPI_Wtime();  // This line can be uncommented to include setup time
  Interp3_Plan interp3_plan;
  interp3_plan.allocate(Q,data_dof);
  interp3_plan.scatter(data_dof, N, isize,istart,Q,g_size,query_points,c_dims,c_comm,interpolation_timer);
  //time+=+MPI_Wtime();  // This line can be uncommented to include setup time
  ...
  PCOUT<<"\n Global Error:\n";
  PCOUT<<"L_inf Error in interpolation of "<<Q*nprocs<<" points is "<<g_err<<'\n';
  PCOUT<<"Rel. L_inf Error in interpolation of "<<Q*nprocs<<" points is "<<g_err/g_norm<<'\n';
  PCOUT<<"\nTotal Time per iteration= "<<g_time/NREP<<"  (for "<<NREP<<" iterations)"<<std::endl;
  PCOUT<<"Interp_timer[0]= "<<g_interpolation_timer[0]/NREP<<" (local interpolation time)"<<std::endl;
  PCOUT<<"Interp_timer[1]= "<<g_interpolation_timer[1]/NREP<<" (communication time)"<<std::endl;
  PCOUT<<"Interp_timer[2]= "<<g_interpolation_timer[2]/NREP<<" (cuda memcpy time)"<<std::endl;
  PCOUT<<"Interp_timer[3]= "<<g_interpolation_timer[3]/NREP<<" (sorting time)"<<std::endl;
