...
PetscErrorCode RegularizationRegistrationH1::EvaluateGradient(VecField* dvR, VecField* v)
{
    ...
    if ( beta[0]  == 0.0  && beta[1] == 0.0 ){
        ...
        for (int i=0; i < 3; ++i){
            n[i]      = static_cast<IntType>(this->m_Opt->m_MiscOpt->N[i]);
...
#pragma omp for
        ...
        // compute inverse fft
        accfft_execute_c2r_t<FFTScaType,ScalarType>(this->m_Opt->m_MiscOpt->plan,this->m_Lv1hat,p_Ainvx1,ffttimers);
        accfft_execute_c2r_t<FFTScaType,ScalarType>(this->m_Opt->m_MiscOpt->plan,this->m_Lv2hat,p_Ainvx2,ffttimers);
        accfft_execute_c2r_t<FFTScaType,ScalarType>(this->m_Opt->m_MiscOpt->plan,this->m_Lv3hat,p_Ainvx3,ffttimers);
        this->m_Opt->IncrementCounter(FFT,3);
