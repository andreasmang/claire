              ...
              Real* query_values,int* c_dims, MPI_Comm c_comm)
{
  ...
  interp3_ghost_xyz_p(ghost_reg_grid_vals, data_dof, N_reg, N_reg_g, isize_g,istart,total_query_points,g_size,all_query_points, all_f_cubic);
...
void rescale_xyz(const int g_size,  int* N_reg, int* N_reg_g, int* istart, const int N_pts, Real* query_points){
  ...
  for(int i=0;i<N_pts;i++){
    query_points[0+COORD_DIM*i]=(query_points[0+COORD_DIM*i]-istart[0]*h[0])*factor[0]+g_size*hp[0];
    query_points[1+COORD_DIM*i]=(query_points[1+COORD_DIM*i]-istart[1]*h[1])*factor[1]+g_size*hp[1];
 ...
 *
 */
void rescale(const int g_size,  int* N_reg, int* N_reg_g, int* istart, const int N_pts, Real* query_points){
...
void interp3_ghost_xyz_p( Real* reg_grid_vals, int data_dof,
    int* N_reg, int* N_reg_g, int * isize_g, int* istart, const int N_pts, const int g_size, Real* query_points_in,
...
void interp3_ghost_p( Real* reg_grid_vals, int data_dof,
    int* N_reg, int* N_reg_g, int * isize_g, int* istart, const int N_pts, const int g_size, Real* query_points_in,
    Real* query_values){
...
void interp3_p( Real* reg_grid_vals, int data_dof,
    ...
    for(int k=0;k<data_dof;k++){
      ...
      for(int j2=0;j2<4;j2++){
        for(int j1=0;j1<4;j1++){
          for(int j0=0;j0<4;j0++){
            //int indx = ((grid_indx[2]+j2)%N_reg) + N_reg*((grid_indx[1]+j1)%N_reg) + N_reg*N_reg*((grid_indx[0]+j0)%N_reg);
            int indx = ((grid_indx[2]+j2)%N_reg[2]) + N_reg[2]*((grid_indx[1]+j1)%N_reg[1]) + N_reg[2]*N_reg[1]*((grid_indx[0]+j0)%N_reg[0]);
            val += M[0][j0]*M[1][j1]*M[2][j2] * reg_grid_vals[indx+k*N_reg3];
          }
        }
      }
...
void interp3_p( Real* reg_grid_vals, int data_dof,
  ...
  for(int i=0;i<N_pts;i++){
    ...
    for(int j=0;j<COORD_DIM;j++){
      ...
      grid_indx[j]=(floor(point[j]))-1;
      point[j]-=grid_indx[j];
      while(grid_indx[j]<0) grid_indx[j]+=N_reg;
    }
    //std::cout<<"grid_index="<<grid_indx[0]<<" "<<grid_indx[1]<<" "<<grid_indx[2]<<std::endl;
 ...
 *
 */
void interp3_p_col( Real* reg_grid_vals, int data_dof,
    int N_reg, const int N_pts, Real*  query_points,
    Real* query_values){
