...
PetscErrorCode RegularizationH1SN::EvaluateGradient(VecField* dvR, VecField* v) {
    ...
    if (beta == 0.0) {
        ...
        // compute forward fft
        this->m_Opt->StartTimer(FFTSELFEXEC);
        ierr = v->GetArraysReadWrite(p_v1, p_v2, p_v3); CHKERRQ(ierr);
        ...
        ierr = v->RestoreArraysReadWrite(p_v1, p_v2, p_v3); CHKERRQ(ierr);
        ...
        EvaluateGradientH1SN_GPU(this->m_v1hat, this->m_v2hat, this->m_v3hat, this->m_Opt->m_FFT.ostart, this->m_Opt->m_FFT.osize, nx, hd*scale*beta);
        ...
        // compute inverse fft
        ierr = dvR->GetArraysReadWrite(p_bv1, p_bv2, p_bv3); CHKERRQ(ierr);
        ...
        ierr = dvR->RestoreArraysReadWrite(p_bv1, p_bv2, p_bv3); CHKERRQ(ierr);
        this->m_Opt->StopTimer(FFTSELFEXEC);
        this->m_Opt->IncrementCounter(FFT, 3);
