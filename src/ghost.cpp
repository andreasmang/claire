...
*/
void ghost_left_right(Real *padded_data,Real* data,int g_size, accfft_plan * plan)
{
  int nprocs, procid;
  ...
  for (int x=0;x<isize[0];++x)
    memcpy(&RS[x*g_size*isize[2]],&data[x*isize[2]*isize[1]+(isize[1]-g_size)*isize[2]],g_size*isize[2]*sizeof(Real));
  ...
  if(procid_r==0) dst_r=nprocs_r-1;
  ...
  for (int x=0;x<isize[0];++x)
    memcpy(&LS[x*g_size*isize[2]],&data[x*isize[2]*isize[1]],g_size*isize[2]*sizeof(Real));
  ...
  if(procid_r==0) dst_s=nprocs_r-1;
  ...
  for (int i=0;i<isize[0];++i){
    memcpy(&padded_data[i*isize[2]*(isize[1]+2*g_size)],&GL[i*g_size*isize[2]],g_size*isize[2]*sizeof(Real));
    memcpy(&padded_data[i*isize[2]*(isize[1]+2*g_size)+g_size*isize[2]],&data[i*isize[2]*isize[1]],isize[1]*isize[2]*sizeof(Real));
    memcpy(&padded_data[i*isize[2]*(isize[1]+2*g_size)+g_size*isize[2]+isize[2]*isize[1]],&GR[i*g_size*isize[2]],g_size*isize[2]*sizeof(Real));
  }
 ...
 * @param[in] plan: AccFFT R2C plan
*/
void ghost_top_bottom(Real *ghost_data,Real* padded_data,int g_size,accfft_plan * plan)
{
  int nprocs, procid;
  MPI_Comm_rank(MPI_COMM_WORLD, &procid);
  MPI_Comm_size(MPI_COMM_WORLD, &nprocs);
  ...
  if(procid_c==0) dst_r=nprocs_c-1;
  ...
  if(procid_c==0) dst_s=nprocs_c-1;
  ...
  memcpy(ghost_data,GT,g_size*isize[2]*(isize[1]+2*g_size)*sizeof(Real));
  memcpy(&ghost_data[g_size*isize[2]*(isize[1]+2*g_size)],padded_data,isize[0]*isize[2]*(isize[1]+2*g_size)*sizeof(Real));
  memcpy(&ghost_data[g_size*isize[2]*(isize[1]+2*g_size)+isize[0]*isize[2]*(isize[1]+2*g_size)],GB,g_size*isize[2]*(isize[1]+2*g_size)*sizeof(Real));
 ...
 * @param[in] plan: AccFFT R2C plan
*/
void ghost_z(Real *ghost_data_z, Real* ghost_data, int g_size,int* isize_g,accfft_plan* plan){

  int * isize=plan->isize;
  for (int i=0;i<isize_g[0];++i)
    for (int j=0;j<isize_g[1];++j){
      memcpy(&ghost_data_z[(i*isize_g[1]+j)*isize_g[2]]                ,&ghost_data[(i*isize_g[1]+j)*isize[2]+isize[2]-g_size],g_size*sizeof(Real));
      memcpy(&ghost_data_z[(i*isize_g[1]+j)*isize_g[2]+g_size]         ,&ghost_data[(i*isize_g[1]+j)*isize[2]],isize[2]*sizeof(Real));
      memcpy(&ghost_data_z[(i*isize_g[1]+j)*isize_g[2]+g_size+isize[2]],&ghost_data[(i*isize_g[1]+j)*isize[2]],g_size*sizeof(Real));
    }
  return;
}
...
size_t accfft_ghost_local_size_dft_r2c(accfft_plan* plan,int g_size, int * isize_g, int* istart_g){
  ...
  if(istart_g[0]<0) istart_g[0]+=n[0];
  if(istart_g[1]<0) istart_g[1]+=n[1];

  isize_g[0]=isize[0]+2*g_size;
  isize_g[1]=isize[1]+2*g_size;
  return (alloc_max+2*g_size*isize[2]*isize[0]*sizeof(Real)+2*g_size*isize[2]*(isize[1]+2*g_size)*sizeof(Real));
}
...
void accfft_get_ghost(accfft_plan* plan,int g_size,int* isize_g,Real* data,Real* ghost_data){
  ...
  if(plan->inplace==true){
    PCOUT<<"accfft_get_ghost_r2c does not support inplace transforms."<<std::endl;
    return;
  }
  ...
  if(g_size>isize[0] || g_size>isize[1]){
    std::cout<<"accfft_get_ghost_r2c does not support g_size greater than isize."<<std::endl;
    return;
  }

  Real *padded_data=(Real*)accfft_alloc(plan->alloc_max+2*g_size*isize[2]*isize[0]*sizeof(Real));
...
size_t accfft_ghost_xyz_local_size_dft_r2c(accfft_plan* plan,int g_size, int * isize_g, int* istart_g){
  ...
  if(istart_g[0]<0) istart_g[0]+=n[0];
  if(istart_g[1]<0) istart_g[1]+=n[1];
  if(istart_g[2]<0) istart_g[2]+=n[2];

  isize_g[0]=isize[0]+2*g_size;
  isize_g[1]=isize[1]+2*g_size;
  isize_g[2]=isize[2]+2*g_size;
  return (alloc_max+2*g_size*isize[2]*isize[0]*sizeof(Real)+2*g_size*isize[2]*isize_g[1]*sizeof(Real)+2*g_size*isize_g[0]*isize_g[1]*sizeof(Real));
}
...
void accfft_get_ghost_xyz(accfft_plan* plan,int g_size,int* isize_g, Real* data,Real* ghost_data){
  ...
  Real *padded_data=(Real*)accfft_alloc(plan->alloc_max+2*g_size*isize[2]*isize[0]*sizeof(Real));
  Real * ghost_data_xy=(Real*) accfft_alloc(plan->alloc_max+2*g_size*isize[2]*isize[0]*sizeof(Real)+2*g_size*isize[2]*isize_g[1]*sizeof(Real));
  ghost_left_right(padded_data,data,g_size,plan);
