...
PetscErrorCode RegularizationH3SN::EvaluateGradient(VecField* dvR, VecField* v) {
    ...
    if (beta == 0.0) {
        ...
        applytime = -MPI_Wtime();
        ierr = EvaluateGradientKernel<3>(this->m_v1hat, this->m_v2hat, this->m_v3hat, this->m_Opt->m_FFT.ostart, this->m_Opt->m_Domain.nx, this->m_Opt->m_FFT.osize, scale*beta, 0); CHKERRQ(ierr);
        applytime += MPI_Wtime();
        timer[FFTHADAMARD] += applytime;
