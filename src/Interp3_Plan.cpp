 ...
 */
void Interp3_Plan::scatter( int data_dof,
    ...
    int* c_dims, MPI_Comm c_comm, double * timings)
{
  ...
  if(this->allocate_baked==false){
    std::cout<<"ERROR Interp3_Plan Scatter called before calling allocate.\n";
    return;
  }
  ...
  rescale_xyz(g_size,N_reg,N_reg_g,istart,total_query_points,all_query_points);
    ...
    Real* query_values,int* c_dims, MPI_Comm c_comm,double * timings)
{
  ...
  MPI_Comm_size(c_comm, &nprocs);
  if(this->allocate_baked==false){
    std::cout<<"ERROR Interp3_Plan interpolate called before calling allocate.\n";
    return;
  }
  if(this->scatter_baked==false){
    std::cout<<"ERROR Interp3_Plan interpolate called before calling scatter.\n";
    return;
  }

  timings[1]+=-MPI_Wtime();
  interp3_ghost_xyz_p(ghost_reg_grid_vals, data_dof, N_reg, N_reg_g, isize_g,istart,total_query_points,g_size,all_query_points, all_f_cubic,true);
  timings[1]+=+MPI_Wtime();
 ...
 */
void Interp3_Plan::slow_run( Real* ghost_reg_grid_vals, int data_dof,
    int* N_reg, int * isize, int* istart, const int N_pts, const int g_size, Real* query_points_in,
